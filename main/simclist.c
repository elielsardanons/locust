...
/*!
 * \internal
 * \brief deletes tmp from list, with care wrt its position (head, tail, middle)
 */
static int list_drop_elem(list_t *restrict l, struct list_entry_s *tmp, unsigned int pos);

/*!
 * \internal
 * \brief set default values for initialized lists
 */
...
/*! 
 * \internal
 * \brief check whether the list internal REPresentation is valid -- Costs O(n)
 */
static int list_repOk(const list_t *restrict l);

/*!
 * \internal
 * \brief check whether the list attribute set is valid -- Costs O(1)
 */
